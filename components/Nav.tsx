...
export default function Nav() {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 px-4 pt-4">
      <div className="max-w-7xl mx-auto h-14 pl-4 pr-3 flex items-center justify-between rounded-full bg-black/95 backdrop-blur-md ring-1 ring-white/15 shadow-lg shadow-black/40">
        ...
        <nav className="hidden md:flex items-center gap-7">
          ...
          <div
            ...
            onBlur={(e) => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setIndOpen(false); }}
          >
            <button
              type="button"
              ...
              onKeyDown={(e) => { if (e.key === "Escape") setIndOpen(false); }}
            ...
            >
              Industries
              <svg aria-hidden="true" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={`mt-0.5 transition-transform ${indOpen ? "rotate-180" : ""}`}>
                <path d="M6 9l6 6 6-6" />
              </svg>
            </button>
            <div
              id="industries-menu"
              className={`absolute left-1/2 -translate-x-1/2 top-full pt-3 transition-all duration-150 ${indOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}
            >
      ...
      {/* Mobile menu */}
      {open && (
        <div id="mobile-menu" className="md:hidden mt-2 max-w-7xl mx-auto rounded-2xl bg-black/95 backdrop-blur-md ring-1 ring-white/10 shadow-lg">
